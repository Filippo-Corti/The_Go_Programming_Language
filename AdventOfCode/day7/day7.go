...
func main() {
	...
	calculateDirectoryDimensions(fileSystem, &sumOfSmallDirs)
	fmt.Println(sumOfSmallDirs)
}

func calculateDirectoryDimensions(fileSystem *Node, sumOfSmallDirs *int) (dim int) {
	for _, node := range fileSystem.next {
		if node.size == 0 {
			//It's a Directory 
			partialDim := calculateDirectoryDimensions(node, sumOfSmallDirs) 
			dim += partialDim
		} else {
			//It's a File
			dim += node.size
