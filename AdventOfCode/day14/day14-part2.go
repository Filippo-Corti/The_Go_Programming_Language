...
func generateGrid(rocks []Point) (grid Grid) {
	...
	rows := maxRow + 3
	cols := 2 * rows + 1
	grid.fallingPoint = Cell{0, 500 - (500 - cols/2), AIR}
	...
	for _, rock := range rocks {
		row := rock.row
		col := rock.col - (500 - cols/2)
		grid.grid[row][col] = Cell{row, col, ROCK}
	}
	//Add the bottom layer of Rocks
	for _, cell := range grid.grid[rows - 1] {
		grid.grid[cell.row][cell.col].fill = ROCK
	}
	return
}
