...
func moveItemsInsideStack(supplyStack [][]string, numberOfItems int, stackToMoveFrom int, stackToMoveTo int) [][]string {
	temp := supplyStack[stackToMoveFrom][0 : numberOfItems]
	itemsToMove := make([]string, len(temp))
	copy(itemsToMove, temp)
	//Add Items to The New Stack (to the top)
	supplyStack[stackToMoveTo] = append(itemsToMove, supplyStack[stackToMoveTo]...)
	//Remove Items from the Old Stack
	supplyStack[stackToMoveFrom] = supplyStack[stackToMoveFrom][numberOfItems:]
	return supplyStack
}